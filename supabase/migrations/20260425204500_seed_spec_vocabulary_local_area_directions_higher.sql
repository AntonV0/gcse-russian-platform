...
upserted_list as (
  ...
  select
    ...
    'higher',
    'Higher tier',
    'Higher tier vocabulary for asking for directions. Higher learners also need the Foundation list for this topic.',
    'local_area_holiday_travel',
    'local_area_holiday_travel_directions',
    ...
    'pearson_edexcel_gcse_russian_1ru0',
    '1RU0',
    'Section 2: Topic-specific vocabulary / Local area, holiday and travel / Asking for directions / Higher tier',
    'section-2:local-area-holiday-and-travel:asking-for-directions:higher:list'
  from upserted_set
  on conflict (source_key, import_key)
  where source_key is not null and import_key is not null
  do update
...
raw_items (
  ...
  values
    (1, $$are you going in a car?$$, $$вы поедете на машине?$$, $$вы поедете на машине?$$, 'phrase', 'not_applicable', 'not_applicable', false),
    (2, $$are you going on foot?$$, $$вы пойдёте пешком?$$, $$вы пойдёте пешком?$$, 'phrase', 'not_applicable', 'not_applicable', false),
    (3, $$cross (over)$$, $$переходить / перейти$$, $$переходить/перейти$$, 'verb', 'not_applicable', 'both', false),
    (4, $$it is 100 metres away$$, $$это в ста метрах отсюда$$, $$это в ста метрах отсюда$$, 'phrase', 'not_applicable', 'not_applicable', false),
    (5, $$take the first road on the left$$, $$вам нужен первый поворот налево$$, $$вам нужен первый поворот налево$$, 'phrase', 'not_applicable', 'not_applicable', false),
    (6, $$turn left$$, $$поверните налево$$, $$поверните налево$$, 'phrase', 'not_applicable', 'not_applicable', false),
    (7, $$turn right$$, $$поверните направо$$, $$поверните направо$$, 'phrase', 'not_applicable', 'not_applicable', false)
),
prepared_items as (
  select
    raw_items.position,
    ...
    (
      'asking-for-directions-higher:item-' ||
