...
with item_lesson_links as (
  ...
  join public.lesson_vocabulary_links links
    ...
    or (
      links.link_type = 'set'
      and links.vocabulary_set_id = items.vocabulary_set_id
      and (
        not exists (
          select 1
          from public.vocabulary_lists lists
          where lists.vocabulary_set_id = items.vocabulary_set_id
        )
        or exists (
          select 1
          from public.vocabulary_lists lists
          ...
          where lists.vocabulary_set_id = items.vocabulary_set_id
            ...
            and (
              lists.tier is null
              or lists.tier in ('both', 'unknown')
              or (links.variant = 'foundation' and lists.tier = 'foundation')
              or (links.variant in ('higher', 'volna') and lists.tier in ('foundation', 'higher'))
            )
        )
      )
    )
      ...
      or exists (
        ...
        where lists.vocabulary_set_id = items.vocabulary_set_id
          ...
          and (
            lists.tier is null
            or lists.tier in ('both', 'unknown')
            or (usages.variant = 'foundation' and lists.tier = 'foundation')
            or (usages.variant in ('higher', 'volna') and lists.tier in ('foundation', 'higher'))
          )
      )
    )
